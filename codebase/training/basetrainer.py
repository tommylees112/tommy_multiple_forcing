...
class BaseTrainer(object):
    ...
    def train_and_validate(self):
        for epoch in range(1, self.cfg["epochs"] + 1):
            ...
            if (self.cfg["learning_rate"] is not None) and (
                epoch in self.cfg["learning_rate"].keys()
            ):
    ...
    def _prepare_train_data(self):
        if (self.cfg["h5_file"] is None) and (self.cfg["scaler_file"] is None):
            ...
            raise RuntimeError(
                "If scaler file is defined, also h5_file has to be specified."
            )
        else:
    ...
    def _create_train_data(self):
        ...
        if self.cfg["dataset"] in ["camels_us", "camels_gb"]:
            create_h5_file(
                basins=self.basins,
                cfg=self.cfg,
