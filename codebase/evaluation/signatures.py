...
def _baseflow_index_jit(
    streamflow: np.ndarray, alpha: float, warmup: int
) -> (float, np.ndarray):
    ...
    for i in range(1, len(streamflow)):
        quickflow1[i] = (
            alpha * quickflow1[i - 1]
            + (1 + alpha) * (streamflow[i] - streamflow[i - 1]) / 2
        )
        if quickflow1[i] > 0:
    ...
    for i in range(len(streamflow) - 2, -1, -1):
        quickflow2[i] = (
            alpha * quickflow2[i + 1]
            + (1 + alpha) * (baseflow1[i] - baseflow1[i + 1]) / 2
        )
    ...
    for i in range(1, len(streamflow)):
        quickflow3[i] = (
            alpha * quickflow3[i - 1]
            + (1 + alpha) * (baseflow2[i] - baseflow2[i - 1]) / 2
        )
        if quickflow3[i] > 0:
...
def baseflow_index(
    da: DataArray, alpha: float = 0.98, warmup: int = 30
) -> (float, DataArray):
...
def slope_fdc(
    da: DataArray, lower_quantile: float = 0.33, upper_quantile: float = 0.66
) -> float:
    ...
    value = (np.log(fdc[idx_lower].values + 1e-8)) - np.log(
        fdc[idx_upper].values + 1e-8
