...
class Tester(object):
    def __init__(
        self, cfg: dict, run_dir: PosixPath, mode: str = "test", init_model: bool = True
    ):
        self.cfg = cfg
        ...
        if mode in ["train", "validation", "test"]:
            ...
            raise ValueError(
                f'Invalid mode {mode}. Must be one of ["train", "validation", "test"]'
            )
    ...
    def evaluate(
        ...
        if self.mode == "validation":
            if (self.cfg["validate_n_random_basins"] is not None) and (
                len(basins) > self.cfg["validate_n_random_basins"]
            ):
        ...
        for basin in pbar:
            ...
            if (
                ds.period_start + pd.DateOffset(days=self.cfg["seq_length"] - 1)
                > self.cfg[f"{self.mode}_start_date"]
            ):
                start_date = ds.period_start + pd.DateOffset(
                    days=self.cfg["seq_length"] - 1
                )
            else:
    ...
    def _create_and_log_figures(self, results, logger, epoch):
        ...
        if qobs_variable_name:
            qobs_variable_name = qobs_variable_name[0]
            max_figures = min(
                self.cfg["validate_n_random_basins"], self.cfg["log_n_figures"]
            )
            for i in range(max_figures):
                ...
                fig, _ = plots.regression_plot(
                    qobs, qsim, title=f"Basin {basins[i]} - Epoch {epoch}"
                )
                figures.append(fig)
    ...
    def _save_results(self, results: dict, epoch: int = None):
        ...
        result_file = (
            self.run_dir / self.mode / weight_file.stem / f"{self.mode}_results.p"
        )
