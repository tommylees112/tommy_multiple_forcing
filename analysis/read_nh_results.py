...
2) Evaluation
    `ipython --pdb neuralhydrology/nh_run.py evaluate -- --run-dir /path/to/run_dir`
3) Extract Results
    `ipython --pdb analysis/read_nh_results -- --run-dir /path/to/run_dir --epoch 30`
...
def get_test_filepath(run_dir: Path, epoch: Optional[int]) -> Path:
    ...
    assert res_fp.exists(), "Has validation been run? ipython --pdb neuralhydrology/nh_run.py evaluate -- --run-dir /path/to/run_dir"
